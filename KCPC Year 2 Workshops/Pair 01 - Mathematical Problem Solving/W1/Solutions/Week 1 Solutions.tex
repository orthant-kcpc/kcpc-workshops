...
\subsection*{Medium -- Two Doors}
Ask either guard: ``Would the other guard say this door is safe?'' If
the chosen door is safe, the truth-teller truthfully reports that the
liar would say no, while the liar lies about the truth-teller's yes
and also says no. If the chosen door is unsafe, both possibilities
produce yes. Therefore the answer identifies the unsafe door; choose
the other one.
...
\subsection*{Basic -- Breaking A Chocolate Bar}
Every break increases the number of pieces by exactly 1. Starting
from 1 piece and ending with $nm$ pieces requires $nm-1$ breaks, and
any sequence of legal breaks achieving unit squares uses that many.
