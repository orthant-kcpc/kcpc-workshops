...
You face two doors. One leads out; one does not. One guard always
tells the truth, and the other always lies. You do not know which
guard is which. You may ask one guard one yes/no question. How can
you identify the safe door?
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item The same cross with the four outer corner squares of the arms removed.
\end{enumerate}

\subsection*{Hard -- Jumping To The Other Side}
On a $5\times6$ board, the 15 cells above a diagonal line are
occupied by counters. A counter may jump over an adjacent counter to
the empty cell immediately beyond it, horizontally, vertically, or
diagonally. Can all counters be moved below the diagonal?
...
An $n\times m$ chocolate bar must be broken into unit squares. Each
break is straight and breaks one existing piece. What is the minimum
number of breaks?
...
at least two of its horizontal/vertical neighbours are infected. What
is the smallest number of initially infected squares needed to
guarantee the whole board becomes infected?
...
A pebble starts at the corner of an infinite grid. A move replaces
one pebble by two pebbles in the cells immediately above and
immediately to the right, if both are empty. Can you clear all
pebbles from the first $n$ diagonals near the corner? Determine all
possible $n$.
